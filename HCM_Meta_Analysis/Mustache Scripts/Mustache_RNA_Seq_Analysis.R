# The RNA-Seq analysis for the output files from Mustache_Overlaps.R
    ...
    # Start of the script
        ...
        # Filtering FitHiC data with the ChIP-Seq Peaks
            ...
            # Filtering the RNA data
                rna_filtered_a <- NULL
                for(i in c((-20000), (-10000), (-5000), (-2500), 0, 2500, 5000, 10000, 20000)){
        ...
        ggplot(rna_filtered, aes(x=-log2(h_mean), y=-log2(d_mean), col=pval)) +
            ...
            write.table(rna_filtered, paste("mustache_rna_analysis_", tf, "_qval", qval, "_.txt", sep=""), col.names=TRUE, row.names=FALSE, quote=FALSE)
            write.table(genes, paste("mustache_gene_list_", tf, "_qval", qval, "_pval_05.txt", sep=""), col.names=TRUE, row.names=FALSE, quote=FALSE)
