...
setwd(export_wd)
df <- read.table("FitHiC_all_qval0.1.txt", header=TRUE, stringsAsFactors=F)
...
q_value <- 0.001
fdr_comb <- df %>% filter(as.numeric(as.character(q.value)) < q_value)



setwd(plot_wd)
   ggplot(fdr_comb, aes(x=-log2(as.numeric(h_contactCount)), y=-log2(as.numeric(d_contactCount)), col=as.numeric(q.value))) +
                                    geom_point(size=0.75) +
                                    theme +
                                    scale_x_continuous(name="-log2 Healthy Contact Counts") +
                                    scale_y_continuous(name="-log2 Disease Contact Counts") +
                                    ggtitle(paste("Disease vs Healthy Counts | all_chr", " | q=", q_value, sep="")) +
                                    labs(color="q.value", size=0.5) +                       
                                    scale_colour_gradientn(colors = viridis(256, option = "D", direction = -1))
                      
                      ggsave(paste("FitHiC_all_chr", "_qval", q_value, ".png", sep="")) 
...
dis <- fdr_comb %>% filter(condition=="disease")
hea <- fdr_comb %>% filter(condition=="healthy")
both <- fdr_comb %>% filter(condition=="both")

nrow(dis);nrow(hea);nrow(both)
